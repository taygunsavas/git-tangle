...
case "$cmd" in
  start)
    if [[ -z "$name" ]]; then echo "[tangle] ERROR: bugfix name required."; exit 1; fi
    ...
    branch="$(tangle::prefix tangle.prefix.bugfix bugfix)/$name"
  ...
  finish)
    ...
    ;;
  *)
    echo "[tangle] Unknown bugfix subcommand: $cmd"; exit 1;;
esac